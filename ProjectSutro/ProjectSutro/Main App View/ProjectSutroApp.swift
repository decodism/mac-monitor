            ...
            CommandMenu("Export telemetry") {
                Section("JSON (pretty)") {
                    ...
                    Button("Selected events \(eventSelection.count > 0 ? ": \(eventSelection.count)" : "")") {
                        ...
                        let filtered = eventSelection.filter {
                            guard let e = systemExtensionManager.coreDataContainer.getEntityByID(id: $0) else { return false }
                            return isEventFiltered(event: e, filteringLongRunningProcs: filteringLongRunningProcs, filterText: filterText, allFilters: allFilters, systemExtensionManager: systemExtensionManager)
                        }

                        systemExtensionManager.coreDataContainer.exportSelectedEvents(eventIDs: Array(filtered))
                ...
                Section("JSONL (lines)") {
                    ...
                    Button("Selected events \(eventSelection.count > 0 ? ": \(eventSelection.count)" : "")") {
                        ...
                        systemExtensionManager.coreDataContainer.exportSelectedEvents(eventIDs: Array(filtered), jsonl: true)
